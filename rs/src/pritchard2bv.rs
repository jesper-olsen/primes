...
impl Wheel {
    ...
    pub fn compress(self: &mut Self) {
        ...
        for i in 1..to+1 {
            //if !self.d[self.w[i]] {
            if !(self.d[self.w[i]/usize::BITS as usize]&(1<<self.w[i]%usize::BITS as usize)!=0) {
                j+=1;
                self.w[j]=self.w[i];
            }
        }
...
fn pritchard2(n:usize) -> impl Iterator<Item=usize> {
    ...
    for i in 1..wheel.w_end+1 {
        ...
        if wheel.w[i]!=0 && !(wheel.d[wheel.w[i]/usize::BITS as usize] & (1<<wheel.w[i]%usize::BITS as usize)!=0) {
